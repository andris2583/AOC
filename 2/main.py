...
def testLevel(data):
  ...
  for j in range(len(data)-1):
    if j == 0:
      asc = data[j] < data[j+1]
    if(asc and (abs(data[j] - data[j+1]) > 3 or abs(data[j] - data[j+1]) < 1 or data[j] > data[j+1])):
      safe_it = False
      break
    if((not asc) and (abs(data[j] - data[j+1]) > 3 or abs(data[j] - data[j+1]) < 1 or data[j] < data[j+1])):
      safe_it = False
      break
  return safe_it
